...
procMatrix3 :: Int -> Int -> ForSyDe.Shallow.Vector (Signal(ForSyDe.Shallow.Matrix (ForSyDe.Shallow.Matrix Double)))-> [Double] --ForSyDe.Shallow.Matrix (ForSyDe.Shallow.Matrix Double) 
procMatrix3 dimx dimy dat = pLists
            where
...
main = do
    ...
    let u3 = vector [intest,inref1,intest,inref3, intest,inref5]

        res = zipWithxSY (procMatrix3 dimx dimy) u3
