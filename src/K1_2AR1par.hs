...
main = do
    ...
    tid1 <- forkIO $ do
        ...
        let u4_s1 = vector [test1,ref1_1,test1,ref2_1]

        let m = zipWithxSY (procMatrix2 dimx dimy) u4_s1
        ...
        writeFile "/home/marcello-costa/workspace/SPL/Out/K1/Lag2/Par/proc1.txt" (show output)
        putMVar m1 output
    ...
    tid2 <- forkIO $ do
        ...
        let u4_s2 = vector [test2,ref1_2,test2,ref2_2]


        let m = zipWithxSY (procMatrix2 dimx dimy) u4_s2
            out = fromSignal m !! 0; mout = fromMatrix out !! 0
            sf = mapSY (spatialFilter dimx dimy) (signal [mout]) -- Spatial Filtering
            output = mapSY (anomaly dimx dimy) (signal [sf])      -- anomaly detection
        writeFile "/home/marcello-costa/workspace/SPL/Out/K1/Lag2/Par/proc2.txt" (show output)
        putMVar m2 output
    ...
    tid3 <- forkIO $ do
        ...
        let u4_s3 = vector [test3,ref1_3,test3,ref2_3]
        




        let m = zipWithxSY (procMatrix2 dimx dimy) u4_s3
            out = fromSignal m !! 0; mout = fromMatrix out !! 0
            sf = mapSY (spatialFilter dimx dimy) (signal [mout]) -- Spatial Filtering
            output = mapSY (anomaly dimx dimy) (signal [sf])      -- anomaly detection
        writeFile "/home/marcello-costa/workspace/SPL/Out/K1/Lag2/Par/proc3.txt" (show output)
        putMVar m3 output
    ...
    tid4 <- forkIO $ do
        ...
        let u4_s4 = vector [test4,ref1_4,test4,ref2_4]

        let m = zipWithxSY (procMatrix2 dimx dimy) u4_s4
            out = fromSignal m !! 0; mout = fromMatrix out !! 0
            sf = mapSY (spatialFilter dimx dimy) (signal [mout]) -- Spatial Filtering
            output = mapSY (anomaly dimx dimy) (signal [sf])      -- anomaly detection
        writeFile "/home/marcello-costa/workspace/SPL/Out/K1/Lag2/Par/proc4.txt" (show output)
        putMVar m4 output
